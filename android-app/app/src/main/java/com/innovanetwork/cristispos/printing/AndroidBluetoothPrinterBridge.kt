...
class AndroidBluetoothPrinterBridge(
    ...
    @JavascriptInterface
    fun selectPrinter(address: String): Boolean = printerManager.selectPrinter(address)

    /** For BT there is no per-device OS permission dialog — this just verifies BLUETOOTH_CONNECT. */
    @JavascriptInterface
    fun requestPermission(): Boolean = printerManager.requestPermission()
